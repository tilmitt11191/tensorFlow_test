...
w_hist = tf.contrib.deprecated.histogram_summary("weights", W)
b_hist = tf.contrib.deprecated.histogram_summary("biases", b)
y_hist = tf.contrib.deprecated.histogram_summary("y", y)
...
writer = tf.summary.FileWriter("../../var/log/tensorflow_log", sess.graph_def)

# 1000回学習
for i in range(1000):
	batch = mnist.train.next_batch(50)
	if i % 10 == 0:
		#result = train_step.run([merged])
		summary_str = sess.run([merged], feed_dict={x: batch[0], y_: batch[1]})
		#summary_str = result[0]
		#acc = result[1]
		acc = ""
		writer.add_summary(summary_str, i)
		#print("i = %s acc = %s W = %s b = %s" % (i, acc, sess.run(W), sess.run(b)))
	else:
